...
public:
    ...
    inline void info(){ std::cout << "Vec : " << vector[0] << ", " << vector[1] << ", " << vector[2] << std::endl; }

    inline array<Real, 3> getConstArray() const { return vector; }
    ...
    inline Vec3< Real > operator+(const Vec3<Real> & v) const{ return Vec3<Real>( vector[0]+v[0], vector[1]+v[1], vector[2]+v[2] ); }
    inline Vec3< Real > operator*(Real factor) { return Vec3<Real>(vector[0]*factor, vector[1]*factor, vector[2]*factor); }
    ...
    inline Vec3< Real > maxVector( Real value ){ return Vec3<Real>( std::max( vector[0], value ), std::max( vector[1], value ), std::max( vector[2], value ));}
    inline Vec3< Real > minVector( Real value ){ return Vec3<Real>( std::min( vector[0], value ), std::min( vector[1], value ), std::min( vector[2], value ));}
};
...
template<class Real>
inline const Vec3<Real>	operator/ ( Real divisor, const Vec3<Real> & vector ){ return Vec3<Real>( vector[0]/divisor, vector[1]/divisor, vector[2]/divisor ); }
...
public:
    ...
    inline array<Real, 2> getConstArray() const { return vector; }
